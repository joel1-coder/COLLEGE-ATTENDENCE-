...
// ✅ NEW: PUT /api/marks/:id  - update an existing marks document by its MongoDB _id
// This is called by MarkRecord.jsx when a teacher edits and saves marks.
// Think of PUT like "updating a record" — we find it by ID and replace its records.
router.put('/:id', authMiddleware, async (req, res) => {
  try {
    ...
    // findById — searches MongoDB for a document with this exact _id
    const doc = await Marks.findById(id);
    if (!doc) {
      return res.status(404).json({ message: 'Marks document not found' });
    }

    // Update each record: if a student already has a mark entry, update it.
    // If not, add a new entry. This is called an "upsert" pattern.
    for (const rec of records) {
      const sid = String(rec.student);
      const existing = doc.records.find(r => String(r.student) === sid);
      if (existing) {
        existing.mark = rec.mark; // Update existing mark
      } else {
        doc.records.push({ student: rec.student, mark: rec.mark }); // Add new
      }
    }
